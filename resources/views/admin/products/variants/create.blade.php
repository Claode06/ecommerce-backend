...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
  ...
  <form action="{{ route('admin.products.variants.store', $product) }}" method="POST">
    ...
    <div class="mb-4">
      ...
      <input type="text" name="label" value="{{ old('label') }}" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-sky-400 outline-none @error('label') border-rose-300 ring-2 ring-rose-200 @enderror">
      @error('label')<p class="text-rose-500 text-xs mt-1">{{ $message }}</p>@enderror
    </div>
    <div class="mb-4">
      <label class="block text-sm font-medium mb-1">Harga</label>
      <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-sky-400 outline-none @error('price') border-rose-300 ring-2 ring-rose-200 @enderror">
      @error('price')<p class="text-rose-500 text-xs mt-1">{{ $message }}</p>@enderror
    </div>
    <div class="mb-4">
      <label class="block text-sm font-medium mb-1">SKU</label>
      <input type="text" name="sku" value="{{ old('sku') }}" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-sky-400 outline-none @error('sku') border-rose-300 ring-2 ring-rose-200 @enderror">
